...
if (!isset($uniq) || $uniq == "") {header("location:"."ExtrHAL_liste_auteurs_extur1.php?erreur=nulteam"); exit;}
//fichier CSV ou txt
if ($_FILES['import']['name'] != "") {
  $ext = strtolower(strrchr($_FILES['import']['name'], '.'));
  if ($ext != ".csv" && $ext != ".txt"){
    header("location:"."ExtrHAL_liste_auteurs_extur1.php?erreur=extfic"); exit;
  }else{
    if ($_FILES['import']['size'] == "0") {
      header("location:"."ExtrHAL_liste_auteurs_extur1.php?erreur=nulfic"); exit;
    }else{
      $temp = $_FILES['import']['tmp_name'];
    }
  }
}else{
  header("location:"."ExtrHAL_liste_auteurs_extur1.php?erreur=nofic"); exit;
}
?>
...
<br><br>
<span class="center">
<a href="./ExtrHAL.php?import=ok&amp;extur1=<?php echo $uniq;?>">Tout est correct : poursuivre avec ExtrHAL</a> - <a href="./ExtrHAL_liste_auteurs_extur1.php">Il y a une/des erreur(s) : recommencer la procédure d'import</a> - <a href="./ExtrHAL.php">Tout annuler et revenir à ExtrHAL</a>
</span>
<br><br>
