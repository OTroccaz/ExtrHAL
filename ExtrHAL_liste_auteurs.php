 ...
 *
 * Liste d'auteurs à mettre en évidence - List of authors to be highlighted
 */
...
if (!in_array($ip, $IP_aut)) {header('Location: https://halur.univ-rennes.fr/');}
?>
...
if (in_array($ip, $IP_aut)) {
  ...
  if (isset($_GET["suppr"]) && $_GET["suppr"] != "") {//Suppression d'une entrée
    ...
    fclose($inF1);
    if ($cehval != "") {
      header('Location: ExtrHAL_liste_auteurs.php?cehval='.$cehval);
    }else{
      header('Location: ExtrHAL_liste_auteurs.php');
    }
  }

  if (isset($_GET["action"]) && $_GET["action"] == "ajout") {//Ajout d'un auteur
    echo '<form method="POST" accept-charset="utf-8" name="ajout" action="ExtrHAL_liste_auteurs.php">';
    ...
    if (isset($_GET["modif"]) && $_GET["modif"] != "") {//Modification d'une entrée
      $modif = $_GET["modif"];
      echo '<form method="POST" accept-charset="utf-8" name="modification" action="ExtrHAL_liste_auteurs.php">';
      ...
      ?>
      <form method="POST" accept-charset="utf-8" name="extrhaliste" action="ExtrHAL_liste_auteurs.php">
      <?php
      //$cehval = "ECOBIO-PAYS";
      ...
      <br><a href="ExtrHAL_liste_auteurs.php?action=ajout&cehval=<?php echo($cehval);?>">Ajouter un auteur</a> -
      ...
      ?>
        <a href="ExtrHAL_liste_auteurs.php?cehval=<?php echo($cehvalimp);?>&action=supprimer" onclick="return confirm('Êtes-vous sûr de vouloir supprimer cette liste ?');">Supprimer cette liste</a>
        - Importer une liste <a href="ExtrHAL_liste_auteurs.php?cehval=<?php echo($cehvalimp);?>&action=importcomplet">entière</a> ou
        compléter avec une liste <a href="ExtrHAL_liste_auteurs.php?cehval=<?php echo($cehvalimp);?>&action=importpartiel">partielle</a> à partir d'un fichier csv ou txt (<em><a href="./modele.csv">cf. modèle</a></em>) -
      <?php
      }else{
      ?>
        Importer une liste <a href="ExtrHAL_liste_auteurs.php?action=importcomplet">entière</a> ou
        compléter avec une liste <a href="ExtrHAL_liste_auteurs.php?action=importpartiel">partielle</a>	 à partir d'un fichier csv ou txt (<em><a href="./modele.csv">cf. modèle</a></em>) -
      <?php
      }
      ?>
      <br><br>
      ...
      ?>
        <form method="POST" accept-charset="utf-8" name="ajout" action="ExtrHAL_liste_auteurs_import.php" enctype="multipart/form-data">
      ...
      foreach($AUTEURS_LISTE AS $i => $valeur) {
        ...
        if ($aff == "oui") {
          ...
          if (isset($cehval)) {
            $text .= '<td valign=top><a href="ExtrHAL_liste_auteurs.php?modif='.$i.'&cehval='.$cehval.'">Modifier</a></td>';
            $text .= '<td valign=top><a href="ExtrHAL_liste_auteurs.php?suppr='.$i.'&cehval='.$cehval.'" onclick="return confirm(\'Êtes-vous sûr de vouloir supprimer cette entrée ?\');">Supprimer</a></td>';
          }else{
            $text .= '<td valign=top><a href="ExtrHAL_liste_auteurs.php?modif='.$i.'">Modifier</a></td>';
            $text .= '<td valign=top><a href="ExtrHAL_liste_auteurs.php?suppr='.$i.'" onclick="return confirm(\'Êtes-vous sûr de vouloir supprimer cette entrée ?\');">Supprimer</a></td>';
          }
          //$text .= '<td valign=top><a href="ExtrHAL_liste_auteurs.php?suppr='.$i.'&cehval='.$cehval.'" onclick="return confirm(\'Êtes-vous sûr de vouloir supprimer cette entrée ?\');">Supprimer</a></td>';
          $text .= '</tr>';
          $chaine1 = mb_ucwords($AUTEURS_LISTE[$i]["nom"]).';';
